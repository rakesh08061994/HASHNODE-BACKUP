...
4. **Zombie Process**: After the child process completes its task, it exits, closing or discarding its resources and environment. However, it leaves behind a `“zombie”` resource, which is an entry in the process table. This entry is cleaned up by the parent process, which is signaled to wake up when the child exits. The parent process then continues with its own program code execution.
...
<div data-node-type="callout">
<div data-node-type="callout-emoji">💡</div>
<div data-node-type="callout-text">How to run existing programs/processes in the background.</div>
</div>

Suppose you run a command Ex: firefox on the foreground, first you have to stop this `(ctrl +z)` and then run this process in the background again to see jobs id using `jobs` command
